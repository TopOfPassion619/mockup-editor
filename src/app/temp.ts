...
export class AppComponent {
  ...
  getImgScreen(event: any) {
    ...
    fabric.Image.fromURL(src_2, (image) => {
      ...
      self.screen_path.scale(this.ratio);
      ...
      fabric.Image.fromURL(src_logo, function(img_logo) {
        ...
        self.screen_group.on('selected', (e) => {
          ...
          d.className += ' show';
          // document.getElementById('x1').value = self.screens.paths[dir_1][0];
          // document.getElementById('y1').value = self.screens.paths[dir_1][1];
          // document.getElementById('x2').value = self.screens.paths[dir_1][2];
          // document.getElementById('y2').value = self.screens.paths[dir_1][3];
          // document.getElementById('x3').value = self.screens.paths[dir_1][4];
          // document.getElementById('y3').value = self.screens.paths[dir_1][5];
          // document.getElementById('x4').value = self.screens.paths[dir_1][6];
          // document.getElementById('y4').value = self.screens.paths[dir_1][7];

          // document.getElementById('a_val').value = self.screens.transformMatrix[dir_1][0];
          // document.getElementById('b_val').value = self.screens.transformMatrix[dir_1][1];
          // document.getElementById('c_val').value = self.screens.transformMatrix[dir_1][2];
          // document.getElementById('d_val').value = self.screens.transformMatrix[dir_1][3];
          // document.getElementById('tx_val').value = self.screens.transformMatrix[dir_1][4];
          // document.getElementById('ty_val').value = self.screens.transformMatrix[dir_1][5];

          // document.getElementById('logox').value = self.screens.logo_pos[dir_1][0];
          // document.getElementById('logoy').value = self.screens.logo_pos[dir_1][1];
          // self.changeAttr();
        });
        self.bringToFront();
      });
  ...
  screen_upload(event) {
    ...
    self.canvas.getObjects().forEach(function(o) {
        if(o.id === self.selected_screen_id) {
          ...
          var svgEl = document.getElementById('svg').getElementsByTagName('svg')[0];
          ...
          var path = fabric.loadSVGFromString(svgStr,function(objects, options) {
            ...
            console.log('crop');
            console.log(self.crop_fx);
            console.log(self.crop_fy);
            console.log(self.crop_lx);
            console.log(self.crop_ly);

            console.log('path');
            console.log(self.path_width);
            console.log(self.path_height);

//             crop
// app.component.ts:814 498.176
// app.component.ts:815 331.15999999999997
// app.component.ts:816 537.648
// app.component.ts:817 311.68
// app.component.ts:819 path
// app.component.ts:820 537.648
// app.component.ts:821 311.68

// crop
// app.component.ts:814 601.4200000000001
// app.component.ts:815 396.905
// app.component.ts:816 331.15999999999997
// app.component.ts:817 180.19
// app.component.ts:819 path
// app.component.ts:820 331.15999999999997
// app.component.ts:821 180.19
            ...
            var ptnGroup = new fabric.PathGroup(objects, {
              top: -self.crop_fy, left: -self.crop_fx, 
              width: self.path_width +  self.crop_lx, 
              height: self.path_height +  self.crop_ly,
          });

            var patternSourceCanvas = new fabric.StaticCanvas();
            patternSourceCanvas.add(ptnGroup);
            patternSourceCanvas.setDimensions({
              width: self.path_width+5000,
              height: self.path_height+5000,
            });
  ...
  getSelectedPathSize() {
    ...
    self.canvas.getObjects().forEach(function(o) {
      if(o.id === self.selected_screen_id) {
        self.path_width = o.item(1).width * self.ratio;
        self.path_height = o.item(1).height * self.ratio;
      }
    });
  }
  readScreenUrl(event) {
    ...
    self.crop_canvas.clear();
    this.crop_canvas.setBackgroundColor({ source: this.props.canvasImage, width: this.crop_canvas.width, height:this.crop_canvas.height, repeat: 'repeat' }, function () {
      // self.props.canvasFill = '';
      self.crop_canvas.renderAll();
    });
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (event) => {
        ...
        fabric.Image.fromURL(self.url, function(myImg) {
          var ww, hh;
          ww = self.crop_canvas.width;
          hh = self.crop_canvas.height;

          console.log( 'ori_crop_canvas_w: ' + ww);
          console.log( 'ori_crop_canvas_h: ' + hh);

          var path_w = self.path_width/2;
          var path_h = self.path_height/2;

          console.log( 'ori_path_w: ' + path_w);
          console.log( 'ori_path_h: ' + path_h);

          var path_ratio = path_w/path_h;
          var crop_w, crop_h;
          var rw = ww / path_w;
          var rh = hh / path_h;
          var r_min = rw;
          if (r_min>rh) r_min = rh;
          ...
          var img_w, img_h, img_r;
           

          var fx, fy, lx, ly;
          
          self.crop_canvas.setWidth( path_w*6);
          self.crop_canvas.setHeight(path_h*6);
          

          fx = self.crop_canvas.width/2 - path_w;
          fy = self.crop_canvas.height/2 - path_h;
          lx = path_w*2;
          ly = path_h*2;

          //i create an extra var for to change some image properties
          self.crop_img = myImg.set({ left: fx, top: fy, fill: 'white' });


          var clipPath = new fabric.Rect({ selectable:false, fill:'transparent', width: lx, top: fy, left: fx, height: ly, fixed: true, strokeWidth: 2, stroke:"red" });
          self.crop_canvas.add(clipPath); 

          self.crop_canvas.add(self.crop_img); 
          self.crop_img.clipPath = new fabric.Rect({ selectable:false, opacity:0, width: lx, top: fy, left: fx, height: ly, fixed: true, strokeWidth: 1, stroke:"red" });
          ...
          self.crop_img.clipTo = function (ctx) {
            ctx.save();
        ...
        //   var logo = img.scale(self.ratio).set({ originX:'center', originY:'center', left: 0, top: 0,width:150,height:150});
        //   self.crop_canvas.add(logo);
        //   self.crop_canvas.renderAll();
        // });
      }
